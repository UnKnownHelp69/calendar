...
def load_schedule():
    ...
    for event in events:
        start_of_week = datetime(year=2025, month=8, day=31, hour=0, minute=0, second=0) # change this if your schedule will be changed to time, when was first week of your semestr
        end_of_week = datetime(year=2025, month=9, day=8, hour=23, minute=59, second=59) # change this if your schedule will be changed to time, when was first week of your semestr
            ...
            for i in range(7):
                ...
                if bd_key in birthdays:
                    if i == 0:
                        new_notifications.append({
                            'type': 'birthday',
                            'message': f'Сегодня день рождения у {birthdays[bd_key]}!',
                            'date': check_date.strftime('%d.%m.%Y')
                        })
                    else:
                        new_notifications.append({
                            'type': 'birthday',
                            'message': f'Через {i} дн. день рождения у {birthdays[bd_key]}',
                            'date': check_date.strftime('%d.%m.%Y')
                        })
    ...
    <div class="container">
        <div class="sidebar">
            ...
            <div class="card">
                ...
                {% for bd_date, name in birthdays.items() %}
            ...
            <div class="card">
                ...
                {% for mark_date, text in marks.items() %}
                <div class="mark-item">
        ...
        function formatDateInput(input, format) {
            ...
            for (let i = 0; i < Math.min(parts.length, 3); i++) {
                if (parts[i].length > 0) {
                    ...
                    } else if (i === 1 && parts[i].length > 2) { // Месяц
                        parts[i] = parts[i].substring(0, 2);
                    } else if (i === 2 && parts[i].length > 4) { // Год
